...
import pandas as pd
import os
from text_summarizer_model import summarize_text
...
for file in pdf_files:
    ...
    for i in range(len(df_table)):
        ...
        tender_title = df_table.iloc[12, 1].strip()
        description = df_table.iloc[12, 1].strip()
        summary_title = summarize_text(tender_title)
        summary_description = summarize_text(description)
        ...
        if not row_exists:
            output_data.append({
                ...
                "Tender Title": summary_title,
                "Tender Description": summary_description,
                "PDF File": file
            })

# Convert the output list to a DataFrame
output_df = pd.DataFrame(output_data, columns=["Sno.", "Tender No.", "Tender Type", "Tender Title", "Tender Description", "PDF File"])
